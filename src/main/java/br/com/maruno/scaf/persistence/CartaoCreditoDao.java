 ...
 * @Data 11 de set de 2018 13:06:14
 * @Pacote br.com.quaemo.scaf.persistence
 * @Nome CartaoCreditoDao.java
 * @NomeCompleto br.com.quaemo.scaf.persistence.CartaoCreditoDao.java
 */
public interface CartaoCreditoDao extends JpaRepository<CartaoCredito, Integer> { 

	@Query("SELECT o FROM CartaoCredito o WHERE o.excluido = false AND o.ativo = true AND o.codUsuario = :codUsuario ORDER BY o.nome")
	List<CartaoCredito> findAtivos(@Param("codUsuario") Integer codUsuario);
	
	@Query("SELECT o FROM CartaoCredito o WHERE o.excluido = false AND o.codUsuario = :codUsuario ORDER BY o.nome")
	List<CartaoCredito> find(@Param("codUsuario") Integer codUsuario);

	CartaoCredito findByCodigoAndCodUsuario(Integer codigo, Integer codUsuario);
